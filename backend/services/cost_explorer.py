...
class CostExplorerService:
    ...
    async def get_costs(self, time_range: str) -> CostData:
        ...
        current_month_costs = await self._get_period_costs(
        ...
        last_month_costs = await self._get_period_costs(
        ...
        service_breakdown = await self._get_service_breakdown(
            current_month_start, end_date
        )
        ...
        avg_daily = (
            sum(dc.total for dc in daily_costs[:7]) / min(7, len(daily_costs))
            if daily_costs
            else 0
        )
        days_in_month = 30
        ...
        budget_utilization = (
            (current_month_costs / monthly_budget) * 100 if monthly_budget > 0 else 0
        )
        ...
        return CostData(
            ...
            today={"total": round(today_cost, 2)},
    ...
    async def _get_daily_costs(self, start_date, end_date) -> list[DailyCost]:
        ...
        try:
            response = self.client.get_cost_and_usage(
                ...
                Filter={"Tags": {"Key": "Project", "Values": ["Harness"]}},
            )
        ...
        try:
            ...
            service_map = {
                "Amazon Elastic Compute Cloud - Compute": (
                    "EC2",
                    "Compute instances for ECS and training",
                ),
                "Amazon Simple Storage Service": (
                    "S3",
                    "Storage for papers and models",
                ),
        ...
        for i in range(6):
            ...
            try:
                response = self.client.get_cost_and_usage(
                    ...
                    Metrics=["UnblendedCost"],
                    GroupBy=[
                        {"Type": "COST_CATEGORY", "Key": "harness-cost-categories"}
                    ],
                    Filter={"Tags": {"Key": "Project", "Values": ["Harness"]}},
                )
    ...
    async def _get_anomalies(self) -> list[CostAnomaly]:
        ...
        try:
            ...
            for anomaly in response.get("Anomalies", []):
                if (
                    anomaly["AnomalyScore"]["CurrentScore"] > 0.7
                ):  # High confidence anomalies
                    service = anomaly.get("RootCauses", [{}])[0].get(
                        "Service", "Unknown"
                    )
                    anomalies.append(
                        CostAnomaly(
    ...
    async def _get_budget_info(self) -> dict[str, float]:
        ...
        try:
            response = self.budgets_client.describe_budget(
                AccountId=self.account_id, BudgetName="harness-monthly-budget"
            )
